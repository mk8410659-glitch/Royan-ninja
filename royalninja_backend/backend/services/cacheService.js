...
const cacheService = {
    ...
    /**
     * Safely flush only royal_ninja cache keys without touching other server databases
     */
    async flushRoyalNinjaCache() {
        if (isRedisAvailable && redisClient) {
            try {
                const patterns = ['battle:*', 'global:*', 'daily_*', 'replay:*', 'rl:*', 'rn:*', 'leaderboard:*', 'wallet:*', 'games:*', 'tasks:*', 'giveaways:*', 'readearn:*', 'quiz:*', 'ninjacatch:*', 'diamondcatch:*'];
                ...
                console.error('⚠️ Error purging royal_ninja Redis keys:', err.message);
            }
        }

        memoryCache.clear();
        console.log('🧹 royal_ninja cache flushed successfully (Redis & In-Memory).');
        return true;
    },

    async flush() {
        return this.flushRoyalNinjaCache();
    },
