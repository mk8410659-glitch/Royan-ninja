...
router.post(['/api/admin/redis-flush', '/redis-flush', '/admin/redis-flush'], adminAuth, async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'royal_ninja Redis & RAM cache reset successfully' });
    } catch (e) {
        res.status(500).json({ success: false, message: e.message });
    }
});
...
router.post('/admin/approve-task-screenshot/:id', adminAuth, checkPermission('dailyTasks'), async (req, res) => {
    try {
        ...
        const User = require('../admin/models/user');
        const formattedAppName = (proof.appName || 'royalninja').toLowerCase().replace(/\s/g, '');
...
router.get('/manage-wallet', adminAuth, checkPermission('wallet'), async (req, res) => {
    try {
        ...
        const defaultRedeemRules = [
            ...
            { taskKey: 'ninja_catch', taskName: 'Ninja Catch', icon: 'assets/icons/emoji.png', minCount: 1, errorMessage: 'Please play Ninja Catch game before redeeming', enabled: false },
...
router.get(['/cpa-docs', '/cpa-postback-docs'], adminAuth, async (req, res) => {
    try {
        ...
        // Determine public API / Postback base URL (Default: https://royalninja.zodplaygames.com)
        let baseUrl = 'https://royalninja.zodplaygames.com';
        ...
        res.render('cpa-docs/index', {
            firebaseServices,
            activeAppName: activeApp.appName || 'royalninja',
            activePage: 'cpa-docs',
            photo: req.admin && req.admin.username ? req.admin.username.charAt(0).toUpperCase() : 'A',
            admin: req.admin,
...
router.all('/cpa-postback', async (req, res) => {
    ...
    const appName = String(params.appName || params.app_name || params.app || 'royalninja').trim();
...
router.get('/s2s-postbacks', adminAuth, async (req, res) => {
    try {
        ...
        res.render('s2s-postbacks/manage', {
            ...
            readEarnTasks,
            playGamesTasks,
            ownPackageId: process.env.PACKAGE_NAME || 'com.royalninja.games'
        });
    } catch (err) {
...
// NINJA CATCH PAGE
router.get(['/ninja-catch', '/diamond-catch'], adminAuth, checkPermission('ninjaCatch', 'diamondCatch', 'dailyTasks', 'appConfig'), async (req, res) => {
    try {
        await connectMongo();
        const appDataDoc = await AppData.findOne({ key: 'appData' }).lean();
        const superOfferConfig = appDataDoc?.config?.superOfferConfig || {};
        ...
        res.render('diamond-catch/manage', {
            config: superOfferConfig,
            activePage: 'ninja-catch',
            photo: req.admin && req.admin.username ? req.admin.username.charAt(0).toUpperCase() : 'A',
            admin: req.admin
        });
    } catch (err) {
        console.error('❌ Error rendering Ninja Catch Page:', err);
        res.status(500).send('Internal Server Error');
    }
});
...
router.post('/app-data-config/app-data', adminAuth, checkPermission('appConfig'), async (req, res) => {
    try {
        ...
        await cacheService.del('global:appData');
        await cacheService.del('ninjacatch:config');
...
router.post('/reward-history/stats', adminAuth, async (req, res) => {
    try {
        ...
        const isRoyalNinja = cleanApp === 'royalninja' || !selectedApp;

        if (isRoyalNinja) {
            const orQuery = [
                { appName: { $regex: /^(royal\s*ninja|royalninja)$/i } },
                { appName: '' },
                { appName: null },
                { appName: { $exists: false } }
            ];
