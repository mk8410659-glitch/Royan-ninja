...
/**
 * Helper to get Ninja Catch / Super Offer config with Redis RAM caching (5 min TTL)
 */
async function getSuperOfferConfig() {
    try {
        const cached = await cacheService.get('ninjacatch:config') || await cacheService.get('diamondcatch:config');
    ...
    try {
        ...
        await cacheService.set('ninjacatch:config', superOfferConfig, ttl);
...
// 1. NINJA / DIAMOND CATCH LIMITS VERIFICATION
router.post(['/ninja-catch-verify', '/diamond-catch-verify', '/verify'], cryptoMiddleware, async (req, res) => {
    try {
...
router.post(['/reward/gems', '/claim-gems', '/gems'], cryptoMiddleware, antiReplayMiddleware, async (req, res) => {
    try {
        ...
        try {
            ...
            trackDailyChallengeProgress(userId, 'ninja_catch', 1);
            trackDailyChallengeProgress(userId, 'diamond_catch', 1);
        } catch (_) {}

        // 🌐 Outgoing Postback Trigger for Ninja Catch Limit Completed (Case 2)
